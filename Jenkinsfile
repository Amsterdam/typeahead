...
node {
    ...
    stage('Test') {
        tryStep "test", {
            ...
            sh "docker-compose run typeahead python -m unittest discover -s /app/typeahead/"
        }, {}
    }

    stage("Build develop image") {
        tryStep "build", {
            def image = docker.build("build.datapunt.amsterdam.nl:5000/datapunt/typeahead:${env.BUILD_NUMBER}")
            image.push()
